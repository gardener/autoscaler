...
func (a *StaticAutoscaler) RunOnce(currentTime time.Time) caerrors.AutoscalerError {
	...
	if len(unregisteredNodes) > 0 {
		...
		// FORK-CHANGE: the log "Some unregistered nodes were removed" conditioned upon the bool value returned from`removeOldUnregisteredNodes`
		// has been removed. In some cases (e.g: when the machine is preserved), the node deletion will not succeed.
		// However, the function would still return `true` – causing the misleading log to be printed.
