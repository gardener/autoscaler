...
func TestMixedNodeInfosProvider(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			// FORK-CHANGE: minSize must be > 0 for cache to be used (fork skips cache for min-size-0 nodegroups)
		...
		{
			// FORK-CHANGE: minSize must be > 0 for cache to be consulted (fork skips cache for min-size-0 nodegroups)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, tg := range tc.groups {
				provider.AddNodeGroup(tg.id, tg.minSize, 0, 0)
