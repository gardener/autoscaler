...
func TestMixedNodeInfosProvider(t *testing.T) {
	...
	type testNodeGroup struct {
		id       string
		minSize  int // FORK-CHANGE: minSize needed to control cache behavior (cache only used when minSize != 0)
		nodes    []testNode
		template *testNode
	}
	...
	testCases := []struct {
		...
		},
		{
			...
			groups: []testNodeGroup{
				{
					id:      "ng",
					minSize: 1, // FORK-CHANGE: minSize must be non-zero for cache to be used
				},
			},
			withCache: map[string]cacheItem{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, tg := range tc.groups {
				provider.AddNodeGroup(tg.id, tg.minSize, 0, 0) // FORK-CHANGE: use tg.minSize to support cache minSize guard
