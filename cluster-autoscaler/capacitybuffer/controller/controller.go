...
func (c *bufferController) configureEventHandlers() {
	...
	// Deployment Informer
	c.client.GetDeploymentInformer().AddEventHandler(c.scalableObjectHandlerFuncs(scalableobject.ApiGroupApps, scalableobject.DeploymentKind)) // #nosec G104 (CWE-703) -- informer registration errors are non-fatal

	// ReplicaSet Informer
	c.client.GetReplicaSetInformer().AddEventHandler(c.scalableObjectHandlerFuncs(scalableobject.ApiGroupApps, scalableobject.ReplicaSetKind)) // #nosec G104 (CWE-703) -- informer registration errors are non-fatal

	// StatefulSet Informer
	c.client.GetStatefulSetInformer().AddEventHandler(c.scalableObjectHandlerFuncs(scalableobject.ApiGroupApps, scalableobject.StatefulSetKind)) // #nosec G104 (CWE-703) -- informer registration errors are non-fatal

	// Job Informer
	c.client.GetJobInformer().AddEventHandler(c.scalableObjectHandlerFuncs(scalableobject.ApiGroupBatch, scalableobject.JobKind)) // #nosec G104 (CWE-703) -- informer registration errors are non-fatal

	// ReplicationController Informer
	c.client.GetReplicationControllerInformer().AddEventHandler(c.scalableObjectHandlerFuncs(scalableobject.ApiGroupCore, scalableobject.ReplicationControllerKind)) // #nosec G104 (CWE-703) -- informer registration errors are non-fatal
}
